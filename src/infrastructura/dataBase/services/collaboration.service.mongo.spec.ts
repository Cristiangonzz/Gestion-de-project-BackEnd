...
describe('collaborationMongoService', () => {
  ...
  describe('findOneBy', () => {
    ...
    const expected =(
      ...
      jest.spyOn(MongoService, 'findOneBy').mockReturnValue(of(mockData) as any);

      // Act
      const result = Repository.findOneBy(collaboration);

      // Assert

      result.subscribe({
        next: (collaboration) => expect(collaboration).toEqual(expected),
        complete: () => { 
          done();
        },
      });
